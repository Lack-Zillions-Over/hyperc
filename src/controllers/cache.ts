...
class Cache extends CacheContract {
   ...
   */
  public async create(identifier: string | number, ttl: number): Promise<boolean> {
    try {
   ...
   */
  public async set<T>(identifier: string | number, key: string, value: T): Promise<boolean> {
    try {
      ...
      await this.client.set(this._nameKey(identifier, key), this.compress<T>(value));
      return true;
    } catch (e) {
      return false;
    }
   ...
   */
  public async get<T>(identifier: string | number, key: string): Promise<T | null> {
    try {
      const isExpired = await this._isExpired(identifier);
  ...
  public async flush(identifier: string | number): Promise<boolean> {
    try {
      const keys = await this.client.keys(`${this.serializeIdentifier(identifier)}-*`);
